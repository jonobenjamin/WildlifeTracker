...
/** Lodge / camp pin using processed kpr_lodges.png (transparent bg + white glow). */
export function lodgeIcon(L, size = 44) {
  const w = size;
  const h = Math.round(size * (112 / 151));
  return L.icon({
    iconUrl: '/data/icons/kpr_lodges.png',
    iconSize: [w, h],
    iconAnchor: [w / 2, h - 2],
    popupAnchor: [0, -(h - 4)],
    className: 'kpr-lodge-icon',
  });
}
